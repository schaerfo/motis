...
api::geocode_response suggestions_to_response(
  ...
  return utl::to_vec(
      suggestions | std::views::filter([&](a::suggestion const& s) {
        return utl::visit(
            s.location_, [](a::address) { return true; },
            [&](a::place_idx_t const p) {
              if (t.place_type_[p] != adr::place_type::kExtra) {
                return true;
              }
              auto const i = adr_extra_place_idx_t{
                  static_cast<adr_extra_place_idx_t::value_t>(p -
                                                              t.ext_start_)};
              auto const available_modes = ae->place_clasz_[i];
              return allowed_modes
                  .transform([available_modes](
                                 n::routing::clasz_mask_t const allowed) {
      ...
      [&](a::suggestion const& s) {
        ...
        for (auto const [i, a] : utl::enumerate(areas)) {
          ...
          api_areas.emplace_back(api::Area{
              ...
              .unique_ =
                  s.unique_area_idx_.has_value() && *s.unique_area_idx_ == i,
              .default_ =
        ...
        return api::Match{
            ...
            .country_ = country_code.and_then([](std::string_view s) {
              return std::optional{std::string{s}};
            }),
            .zip_ = s.zip_area_idx_.and_then([&](unsigned const zip_area_idx) {
              return std::optional<std::string>{
                  t.strings_[t.area_names_[areas[zip_area_idx]]
                                          [a::kDefaultLangIdx]]
                      .view()};
            }),
            .tz_ = s.tz_ == a::timezone_idx_t::invalid()
                       ? std::nullopt
                       : std::optional{std::string{
                             t.timezone_names_[s.tz_].view()}},
